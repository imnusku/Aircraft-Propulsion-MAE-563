...
% Initialize Mach number M2 range
M2_range = 0.1:0.1:2.5;

% Preallocate arrays for results
overall_efficiency_results = zeros(size(M2_range));
thrust_results = zeros(size(M2_range));
TSFC_results = zeros(size(M2_range));

% Fixed parameters
z = 4300; % Altitude in meters
M1 = 2.4; % Fixed Mach number at inlet
eta_d = 0.92; % Fixed inlet/diffuser efficiency
eta_n = 0.94; % Fixed nozzle efficiency

for idx = 1:length(M2_range)
    M2 = M2_range(idx); % Current Mach number entering the combustor
...
%% Plot the results
figure;
plot(M2_range, overall_efficiency_results, 'LineWidth', 2);
xlabel('Mach Number (M2)');
ylabel('Overall Efficiency (\eta_o)');
title('Overall Efficiency vs. Mach Number (M2)');
grid on;

figure;
plot(M2_range, thrust_results, 'LineWidth', 2);
xlabel('Mach Number (M2)');
ylabel('Thrust (N)');
title('Thrust vs. Mach Number (M2)');
grid on;

figure;
plot(M2_range, TSFC_results, 'LineWidth', 2);
xlabel('Mach Number (M2)');
ylabel('TSFC (kg/hr/N)');
title('TSFC vs. Mach Number (M2)');
grid on;
