...
% Initialize altitude number range
z_range=2000:1:30000; 

% Preallocate arrays for results
overall_efficiency_results = zeros(size(z_range));
thrust_results = zeros(size(z_range));
TSFC_results = zeros(size(z_range));

for idx = 1:length(z_range)
...
%% Plot the results
figure;
plot(z_range, overall_efficiency_results, 'LineWidth', 2);
xlabel('Altitude');
ylabel('Overall Efficiency');
title('Overall Efficiency vs. Altitude');
grid on;

figure;
plot(z_range, thrust_results, 'LineWidth', 2);
xlabel('Altitude');
ylabel('Thrust (N)');
...
figure;
plot(z_range, TSFC_results, 'LineWidth', 2);
xlabel('Altitude');
ylabel('TSFC (kg/hr/N)');
