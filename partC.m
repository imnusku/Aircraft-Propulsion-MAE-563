...
% Initialize Mach number range
M1_range = 0.8:0.1:5.0;

% Preallocate arrays for results
overall_efficiency_results = zeros(size(M1_range));
thrust_results = zeros(size(M1_range));
TSFC_results = zeros(size(M1_range));

for idx = 1:length(M1_range)
    % Set M1 for this iteration
    M1 = M1_range(idx);
    ...
    if Tt3_choked < Tt3_max
        ...
        M3_squared = M3_roots(M3_roots > 0 & M3_roots < 2);
        M3 = sqrt(M3_squared);
    end
...
%% Plot the results
figure;
plot(M1_range, overall_efficiency_results, 'LineWidth', 2);
xlabel('Mach Number (M1)');
ylabel('Overall Efficiency');
title('Overall Efficiency vs. Mach Number');
grid on;

figure;
plot(M1_range, thrust_results, 'LineWidth', 2);
xlabel('Mach Number (M1)');
ylabel('Thrust (N)');
...
figure;
plot(M1_range, TSFC_results, 'LineWidth', 2);
xlabel('Mach Number (M1)');
ylabel('TSFC (kg/hr/N)');
