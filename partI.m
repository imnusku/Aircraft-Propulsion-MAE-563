...
% Define the range for parametric analysis
M2_range = 0.1:0.01:5; % Diffuser exit Mach number
Tt3_range = 1500:5:2400; % Combustor exit total temperature

% Preallocate results
Thrust = zeros(length(M2_range), length(Tt3_range));
Efficiency = zeros(length(M2_range), length(Tt3_range));

% Loop over M2 and Tt3 to calculate thrust and efficiency
for i = 1:length(M2_range)
    M2 = M2_range(i);
    for j = 1:length(Tt3_range)
        Tt3 = Tt3_range(j);
        %% module 1
...
overall_efficiency = thermal_efficiency * propulsive_efficiency;
            ...
            Thrust(i, j) = total_thrust;
            Efficiency(i, j) = overall_efficiency;
    end
end

% Find optimal values
[max_thrust, idx_thrust] = max(Thrust(:));
[optimal_i_thrust, optimal_j_thrust] = ind2sub(size(Thrust), idx_thrust);
optimal_M2_thrust = M2_range(optimal_i_thrust);
optimal_Tt3_thrust = Tt3_range(optimal_j_thrust);

[max_efficiency, idx_efficiency] = max(Efficiency(:));
[optimal_i_efficiency, optimal_j_efficiency] = ind2sub(size(Efficiency), idx_efficiency);
optimal_M2_efficiency = M2_range(optimal_i_efficiency);
optimal_Tt3_efficiency = Tt3_range(optimal_j_efficiency);

% Display results
fprintf('Maximum Thrust: %.2f N at M2 = %.2f, Tt3 = %.2f K\n', max_thrust, optimal_M2_thrust, optimal_Tt3_thrust);
fprintf('Maximum Efficiency: %.4f at M2 = %.2f, Tt3 = %.2f K\n', max_efficiency, optimal_M2_efficiency, optimal_Tt3_efficiency);
