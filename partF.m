...
% Initialize inlet/diffuser efficiency range
eta_d_range = 0.5:0.05:1.0;

% Preallocate arrays for results
overall_efficiency_results = zeros(size(eta_d_range));
thrust_results = zeros(size(eta_d_range));
TSFC_results = zeros(size(eta_d_range));

% Fixed parameters
z = 4300; % Altitude in meters
M1 = 2.4; % Mach number

for idx = 1:length(eta_d_range)
    eta_d = eta_d_range(idx); % Current diffuser efficiency
...
%% Plot the results
figure;
plot(eta_d_range, overall_efficiency_results, 'LineWidth', 2);
xlabel('Inlet/Diffuser Efficiency (\eta_d)');
ylabel('Overall Efficiency');
title('Overall Efficiency vs. Inlet/Diffuser Efficiency');
grid on;

figure;
plot(eta_d_range, thrust_results, 'LineWidth', 2);
xlabel('Inlet/Diffuser Efficiency (\eta_d)');
ylabel('Thrust (N)');
title('Thrust vs. Inlet/Diffuser Efficiency');
grid on;

figure;
plot(eta_d_range, TSFC_results, 'LineWidth', 2);
xlabel('Inlet/Diffuser Efficiency (\eta_d)');
ylabel('TSFC (kg/hr/N)');
title('TSFC vs. Inlet/Diffuser Efficiency');
grid on;
